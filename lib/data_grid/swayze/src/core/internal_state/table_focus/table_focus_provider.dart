...
abstract class TableFocus implements ValueListenable<TableFocusState> {
  static TableFocus of(BuildContext context) {
    return context.findAncestorWidgetOfExactType<_TableFocusProviderScope>()!.state;
  }
...
class TableFocusProvider extends StatefulWidget {
  ...
  const TableFocusProvider({Key? key, required this.child, required this.focusNode, required this.focusScopeNode}) : super(key: key);

  @override
  _TableFocusProviderState createState() => _TableFocusProviderState();
}

class _TableFocusProviderState extends State<TableFocusProvider> implements TableFocus {
  /// A internal [ValueNotifier] that actually keeps the state.
  late final _notifier = ValueNotifier(
    TableFocusState(hasFocus: widget.focusNode.hasFocus, isActive: widget.focusScopeNode.focusedChild == widget.focusNode),
  );
  ...
  Widget build(BuildContext context) {
    return _TableFocusProviderScope(state: this, child: widget.child);
  }
}
...
class _TableFocusProviderScope extends InheritedWidget {
  ...
  const _TableFocusProviderScope({Key? key, required this.state, required Widget child}) : super(key: key, child: child);
