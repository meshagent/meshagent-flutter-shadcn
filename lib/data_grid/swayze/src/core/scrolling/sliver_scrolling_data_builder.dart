...
typedef ContentWithScrollingDataBuilder = Widget Function(BuildContext context, ScrollingData constraints);
...
class SliverScrollingDataBuilder extends StatelessWidget {
  ...
  const SliverScrollingDataBuilder({Key? key, required this.leadingPadding, required this.extent, required this.contentBuilder})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SliverLayoutBuilder(
      builder: (BuildContext context, SliverConstraints constraints) {
        ...
        return _SliverFixed(
          sliver: SliverToBoxAdapter(child: contentBuilder(context, scrollingData)),
          extent: extent,
        );
      },
    );
...
class _SliverFixed extends SingleChildRenderObjectWidget {
  ...
  const _SliverFixed({required Widget sliver, required this.extent}) : super(child: sliver);
  ...
  @override
  void updateRenderObject(BuildContext context, RenderSliverFixedBoxAdapter renderObject) {
...
class RenderSliverFixedBoxAdapter extends RenderProxySliver {
  RenderSliverFixedBoxAdapter(this._extent, [RenderSliver? child]) : super(child);
...
class ScrollingData {
  ...
  late final double viewportExtent = (totalExtent - constraints.scrollOffset).clamp(0.0, constraints.viewportMainAxisExtent);
  ...
  @visibleForTesting
  ScrollingData.fromSliverConstraints({required this.leadingPadding, required this.constraints, required this.totalExtent});
  ...
  @override
  int get hashCode => constraints.hashCode ^ totalExtent.hashCode ^ viewportExtent.hashCode;
