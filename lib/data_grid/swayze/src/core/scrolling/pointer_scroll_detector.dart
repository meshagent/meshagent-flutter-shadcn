...
class PointerScrollDetector extends StatelessWidget {
  ...
  void handlePointerSignal(PointerSignalEvent event) {
    ...
    final horizontalDelta = _handlePointerSignalAxis(event, horizontalPosition, Axis.horizontal);

    final verticalDelta = _handlePointerSignalAxis(event, verticalPosition, Axis.vertical);

    if (verticalDelta != null || horizontalDelta != null) {
      GestureBinding.instance.pointerSignalResolver.register(event, applyScroll);
    }
  }
  ...
  double? _handlePointerSignalAxis(PointerScrollEvent event, ScrollPosition position, Axis axis) {
    final shouldAccept = position.pixels != 0.0 || position.minScrollExtent != position.maxScrollExtent;
  ...
  void applyScroll(PointerEvent event) {
    ...
    final horizontalDelta = _pointerSignalEventDelta(event, Axis.horizontal);
    final verticalDelta = _pointerSignalEventDelta(event, Axis.vertical);
  ...
  @override
  Widget build(BuildContext context) => Listener(onPointerSignal: handlePointerSignal, behavior: HitTestBehavior.opaque, child: child);
}
