...
typedef VirtualizationStateWidgetBuilder = Widget Function(BuildContext context, VirtualizationState virtualizationState);
...
class _VirtualizationCalculatorState extends State<VirtualizationCalculator> implements VirtualizationState {
  late final controller = InternalScope.of(context).controller;

  late final headerController = controller.tableDataController.getHeaderControllerFor(axis: axis);
  ...
  void initState() {
    ...
    final initialVirtualization = calcVirtualizationMemoized(offset, extent, headerController.value);
  ...
  void didUpdateWidget(covariant VirtualizationCalculator oldWidget) {
    ...
    if (oldWidget.scrollingData != widget.scrollingData || oldWidget.frozenAmount != widget.frozenAmount) {
      ...
      final virtualizationResult = calcVirtualizationMemoized(offset, extent, headerController.value);
  ...
  late final VirtualizationCalcResult Function(double, double, SwayzeHeaderState) calcVirtualizationMemoized = memo3((
    double offset,
    double extent,
    SwayzeHeaderState headerState,
  ) {
    return pixelRangeToHeaderRange(
      offset,
      offset + extent,
      HeaderComputationAuxiliaryData.fromHeaderState(axis: widget.axis, headerState: headerController.value),
    );
  });
