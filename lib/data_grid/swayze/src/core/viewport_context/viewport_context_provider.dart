...
import '../virtualization/virtualization_calculator.dart' show VirtualizationCalculator, VirtualizationState;
...
class ViewportContextProvider extends StatefulWidget {
  ...
  @override
  _ViewportContextProviderState createState() => _ViewportContextProviderState();

  /// From descendants of [_ViewportContextProviderScope], recover the nearest
  /// [ViewportContext].
  static ViewportContext of(BuildContext context) {
    return context.findAncestorWidgetOfExactType<_ViewportContextProviderScope>()!.state;
  }
}

class _ViewportContextProviderState extends State<ViewportContextProvider> implements ViewportContext {
  ...
  late final ValueNotifier<Range> horizontalRangeNotifier = widget.horizontalVirtualizationState.rangeNotifier;

  /// A value notifier to subscribe to changes in the visible rows.
  /// It is generated by the ancestor [VirtualizationCalculator].
  late final ValueNotifier<Range> verticalRangeNotifier = widget.verticalVirtualizationState.rangeNotifier;

  @override
  late final _ChangeableViewportHeaderAxisState columns = _ChangeableViewportHeaderAxisState(
    Axis.horizontal,
    widget.horizontalVirtualizationState,
  );

  @override
  late final _ChangeableViewportHeaderAxisState rows = _ChangeableViewportHeaderAxisState(
    Axis.vertical,
  ...
  @override
  _ChangeableViewportHeaderAxisState getAxisContextFor({required Axis axis}) {
  ...
  void updateAxis(Axis axis) {
    ...
    final rangeNotifier = Axis.horizontal == axis ? horizontalRangeNotifier : verticalRangeNotifier;
  ...
  PositionResult pixelToPosition(double pixelOffset, Axis axis) {
    ...
    final trailingEdge = axisContextState.extent + axisContextState.frozenExtent;
    ...
    if (pixelOffset - virtualizationState.displacement.abs() <= axisContextState.frozenExtent) {
      final position = _pixelOffsetToIndex(pixelOffset - virtualizationState.displacement.abs(), axisContextState.frozenOffsets);

      return PositionResult(position: position, overflow: overflow, axis: axis, isFrozen: true);
    }
    ...
    final position = range.start + _pixelOffsetToIndex(pixelOffset, headerCollection);

    return PositionResult(position: position, overflow: overflow, axis: axis, isFrozen: false);
  }
  ...
  PixelResult positionToPixel(int globalPosition, Axis axis, {required bool isForFrozenPanes}) {
    ...
    final range = isForFrozenPanes ? stateValue.frozenRange : stateValue.scrollableRange;
    final headerCollection = isForFrozenPanes ? stateValue.frozenOffsets : stateValue.offsets;
    final leadingEdge = isForFrozenPanes ? 0.0 : stateValue.frozenExtent;
    final trailingEdge = isForFrozenPanes ? stateValue.frozenExtent : stateValue.extent;

    if (range.isNil) {
      final virtualizationState = getAxisContextFor(axis: axis).virtualizationState;
      final hasScrolledPastIt = virtualizationState.scrollingData.constraints.scrollOffset > 0;
      return PixelResult(
        axis: axis,
        offscreenDetails: hasScrolledPastIt ? OffscreenDetails.leading : OffscreenDetails.trailing,
        pixel: 0.0,
      );
    }
    ...
    if (isOnScreen) {
      final localPosition = (globalPosition - range.start).clamp(0, range.iterable.length - 1);
      final offset = headerCollection.elementAt(localPosition);

      return PixelResult(axis: axis, offscreenDetails: OffscreenDetails.noOverflow, pixel: offset);
    }

    if (globalPosition < range.start) {
      return PixelResult(axis: axis, offscreenDetails: OffscreenDetails.leading, pixel: leadingEdge);
    }

    return PixelResult(axis: axis, offscreenDetails: OffscreenDetails.trailing, pixel: trailingEdge);
  }
  ...
  CellPositionResult getCellPosition(IntVector2 globalPosition) {
    ...
    if (isOffscreenX || isOffscreenY) {
      return CellPositionResult.offscreen(isOffscreenX: isOffscreenX, isOffscreenY: isOffscreenY);
    }
    ...
    return CellPositionResult(leftTop: Offset(left, top), cellSize: Size(width, height));
  }
...
class _ViewportContextProviderScope extends InheritedWidget {
  ...
  const _ViewportContextProviderScope({Key? key, required this.state, required Widget child}) : super(key: key, child: child);
...
class _ChangeableViewportHeaderAxisState extends ViewportAxisContext {
  _ChangeableViewportHeaderAxisState(Axis axis, VirtualizationState virtualizationState) : super(axis, virtualizationState);
