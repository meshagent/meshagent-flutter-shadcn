...
/// widget tree.
class SwayzeScrollController<ParentType extends SwayzeController> extends SubController<ParentType> {
  ...
  bool get isAttached => horizontalScrollController != null && verticalScrollController != null && _internalContext != null;

  /// Shorthand access to the internal [ViewportContext]
  ViewportContext? get _viewportContext => _internalContext != null ? ViewportContextProvider.of(_internalContext!) : null;
  ...
  double _precedingPixels(Axis axis) =>
      axis == Axis.horizontal ? 0.0 : _viewportContext!.rows.virtualizationState.scrollingData.constraints.precedingScrollExtent;

  /// Get a [ScrollController] for the specified [axis].
  ScrollController? getScrollControllerFor({required Axis axis}) {
    return axis == Axis.horizontal ? horizontalScrollController : verticalScrollController;
  }

  /// Get a [AutoScrollController] for the specified [axis].
  AutoScrollController? getAutoScrollControllerFor({required Axis axis}) {
    return axis == Axis.horizontal ? horizontalAutoScrollController : verticalAutoScrollController;
  }

  /// Returns true if any of the [AutoScrollController] is instantiated.
  bool get isAutoScrollOn => horizontalAutoScrollController != null || verticalAutoScrollController != null;
  ...
  Future<void> ensureTableVisibility({
    ...
    final tableRenderObject = _internalContext!.findRootAncestorStateOfType<SliverSwayzeTableState>()!.context.findRenderObject()!;
  ...
  double? _getOffsetToHeader(int index, Axis axis, ScrollController scrollController) {
    final headersState = parent.tableDataController.getHeaderControllerFor(axis: axis).value;
    ...
    final headerState = parent.tableDataController.getHeaderControllerFor(axis: axis).value;
    ...
    final virtualizationState = viewportContext.getAxisContextFor(axis: axis).virtualizationState;

    final isForFrozenPanes = viewportContext.getAxisContextFor(axis: axis).value.frozenRange.contains(index);

    // Define the position of the header in relation to the current viewport.
    final offscreenDetails = viewportContext.positionToPixel(index, axis, isForFrozenPanes: isForFrozenPanes).offscreenDetails;
    ...
    if (!offscreenDetails.isOffscreen && !visibleRange.isNil) {
      ...
      if (index == visibleRange.start && virtualizationState.displacement == 0.0) {
      ...
      scrollToTheViewportLeadingEdge = offscreenDetails == OffscreenDetails.leading;
    }
    ...
    if (scrollToTheViewportLeadingEdge) {
      ...
      final viewportBottomEdge = scrollController.position.pixels + virtualizationState.scrollingData.constraints.viewportMainAxisExtent;

      final tableBottomEdge = preceedingPixels + headerState.extent + virtualizationState.scrollingData.leadingPadding;
      ...
      viewportCompensation =
          virtualizationState.scrollingData.constraints.viewportMainAxisExtent - virtualizationState.scrollingData.leadingPadding;
    ...
    final targetOffset = getAxisHeaderOffset(
      indexToDiscoverOffset,
      HeaderComputationAuxiliaryData.fromHeaderState(axis: axis, headerState: headerState),
    );

    // Finally, sum up to achieve the real offset of a header.
    final absolutePosition = preceedingPixels + targetOffset - viewportCompensation;
  ...
  Future<void> animateToHeader(
    ...
    // Finally, apply offset
    return scrollController.animateTo(animationOffset, duration: duration, curve: curve);
  }
  ...
  Future<void> animateToCoordinate(
    ...
    animateToHeader(coordinate.dx, Axis.horizontal, duration: duration, curve: curve),
    animateToHeader(coordinate.dy, Axis.vertical, duration: duration, curve: curve),
  ]);
  ...
  /// the scrollable area.
  void startOrUpdateAutoScroll({required AxisDirection direction, required double pointerDistance, double? maxToScroll}) {
    ...
    final newAutoScrollController = AutoScrollController(pointerDistance: pointerDistance);
...
class ScrollControllerAttacher extends StatefulWidget {
  ...
  @override
  _ScrollControllerAttacherState createState() => _ScrollControllerAttacherState();
}

class _ScrollControllerAttacherState extends State<ScrollControllerAttacher> {
  late final SwayzeScrollController swayzeScrollController = InternalScope.of(context).controller.scroll;
