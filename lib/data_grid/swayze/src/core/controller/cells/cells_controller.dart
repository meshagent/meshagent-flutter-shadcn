...
/// [CellDataType]
typedef SwayzeCellsControllerCellParser<CellDataType extends SwayzeCellData> = CellDataType Function(dynamic rawCell);
...
///   updates the UI.
class SwayzeCellsController<CellDataType extends SwayzeCellData> extends ChangeNotifier implements DataController, SubController {
  @override
  ...
  SwayzeCellsController({required this.parent, required this.cellParser, required Iterable<dynamic> initialRawCells}) {
    for (final initialCell in initialRawCells) {
      PutCellOperation<CellDataType>(cellParser(initialCell)).commit(_cellMatrix);
    }
  }
  ...
  final _cellOperationsListeners = <ValueChanged<List<CellOperation>>, VoidCallback>{};
  ...
  void removeCellOperationsListener(ValueChanged<List<CellOperation>> listener) {
  ...
  void updateState(void Function(CellsModifier<CellDataType> modifier) stateUpdate) {
  ...
  /// (the last empty).
  IntVector2 getNextCoordinateInCellsBlock({required IntVector2 originalCoordinate, required AxisDirection direction}) {
    ...
    final headerController = parent.tableDataController.getHeaderControllerFor(axis: axis);
    final limit = parent.tableDataController.allowElasticExpansion
        ? headerController.value.totalCount - 1
        : parent.tableDataController.getLastTableHeaderIndex(axis: axis);

    final isFirstNeighbourCellFilled = _cellMatrix[currentCoordinate]?.hasVisibleContent == true;
    ...
    do {
      ...
      currentCoordinate = IntVector2(
        axis == Axis.horizontal ? max(0, min(currentCoordinate.dx, limit)) : currentCoordinate.dx,
        axis == Axis.vertical ? max(0, min(currentCoordinate.dy, limit)) : currentCoordinate.dy,
      );
    } while (shouldContinueLookup(previousCoordinate, currentCoordinate));

    final currentCellIsEmpty = _cellMatrix[currentCoordinate]?.hasNoVisibleContent == true;

    return (isFirstNeighbourCellFilled || currentCellIsEmpty) ? previousCoordinate : currentCoordinate;
  }

  /// Given a [IntVector2] and a [AxisDirection] return the next coordinate
  /// that is not hidden.
  IntVector2 getNextCoordinate({required IntVector2 originalCoordinate, required AxisDirection direction}) {
    final axis = axisDirectionToAxis(direction);
    final headerController = parent.tableDataController.getHeaderControllerFor(axis: axis);
    var newCoordinate = originalCoordinate;
    final limit = parent.tableDataController.getLastTableHeaderIndex(axis: axis);
    ...
    do {
      ...
      newCoordinate = IntVector2(
        axis == Axis.horizontal
            ? max(0, parent.tableDataController.allowElasticExpansion ? newCoordinate.dx : min(newCoordinate.dx, limit))
            : newCoordinate.dx,
        axis == Axis.vertical
            ? max(0, parent.tableDataController.allowElasticExpansion ? newCoordinate.dy : min(newCoordinate.dy, limit))
            : newCoordinate.dy,
      );
