...
class UserSelectionState {
  ...
  static UserSelectionState get initial => UserSelectionState._(
    BuiltList.from(<UserSelectionModel>[
      CellUserSelectionModel.fromAnchorFocus(anchor: const IntVector2.symmetric(0), focus: const IntVector2.symmetric(0)),
    ]),
  ...
  /// Whether the current selection should be rendered or highlighted.
  final bool hasVisibleSelection;

  Iterable<UserSelectionModel> get visibleSelections {
    if (!hasVisibleSelection) {
      return const <UserSelectionModel>[];
    }
    return selections;
  }
  ...
  UserSelectionModel get primarySelection => selections.elementAt(_primaryIndex);
  ...
  const UserSelectionState._(this.selections, {required this.hasVisibleSelection}) : _primaryIndex = selections.length - 1;
  ...
  UserSelectionState addSelection(UserSelectionModel newSelection) {
    final newSelections = selections.rebuild((builder) => builder.add(newSelection));

    return UserSelectionState._(newSelections, hasVisibleSelection: true);
  }

  /// Replace the current selection list while preserving its ordering, which
  /// keeps the last item as the primary selection.
  UserSelectionState replaceSelections(Iterable<UserSelectionModel> newSelections) {
    return UserSelectionState._(BuiltList<UserSelectionModel>.from(newSelections), hasVisibleSelection: hasVisibleSelection);
  }
  ...
  UserSelectionState resetSelectionsToACellSelection({required IntVector2 anchor, required IntVector2 focus}) {
    final newList = BuiltList<UserSelectionModel>.from(<UserSelectionModel>[
      CellUserSelectionModel.fromSelectionModel(primarySelection, anchor: anchor, focus: focus),
    ]);
    return UserSelectionState._(newList, hasVisibleSelection: true);
  }
  ...
  UserSelectionState resetSelectionsToHeaderSelection({required Axis axis, required int anchor, required int focus}) {
    final newList = BuiltList<UserSelectionModel>.from(<UserSelectionModel>[
      HeaderUserSelectionModel.fromSelectionModel(primarySelection, anchor: anchor, focus: focus, axis: axis),
    ]);
    return UserSelectionState._(newList, hasVisibleSelection: true);
  }
  ...
  UserSelectionState updateLastSelectionToCellSelection({IntVector2? anchor, required IntVector2 focus}) {
    ...
    builder.removeLast();
    builder.add(CellUserSelectionModel.fromSelectionModel(lastSelection, anchor: effectiveAnchor, focus: focus));
    return UserSelectionState._(builder.build(), hasVisibleSelection: true);
  }
  ...
  /// [anchor] defaults to the last selections
  /// [UserSelectionModel.anchorCoordinate].
  UserSelectionState updateLastSelectionToHeaderSelection({int? anchor, required int focus, required Axis axis}) {
    final lastSelection = selections.last;
    final effectiveAnchor = anchor ?? _vectorValueFromAxis(lastSelection.anchorCoordinate, axis);

    final builder = selections.toBuilder();
    builder.removeLast();

    builder.add(HeaderUserSelectionModel.fromSelectionModel(lastSelection, axis: axis, anchor: effectiveAnchor, focus: focus));
    return UserSelectionState._(builder.build(), hasVisibleSelection: true);
  }

  /// Converts the last [selections]'s to a new [TableUserSelectionModel]
  UserSelectionState resetSelectionsToTableSelection() {
    final newList = BuiltList<UserSelectionModel>.from(<UserSelectionModel>[TableUserSelectionModel.fromSelectionModel(primarySelection)]);
  ...
  UserSelectionState expandLastSelection(AxisDirection direction, {required IntVector2 Function(IntVector2 coordinate) getNextCoordinate}) {
    ...
    return updateLastSelectionToCellSelection(focus: newFocus, anchor: selection.anchor);
  }
  ...
  UserSelectionState expandLastSelectionByBlock(
    ...
    required IntVector2 Function(IntVector2 coordinate) getNextCoordinateInCellsBlock,
    required int limit,
  }) {
    if (primarySelection is TableUserSelectionModel) {
      return this;
    ...
    if (primarySelection is HeaderUserSelectionModel) {
      ...
      final focus = direction == AxisDirection.up || direction == AxisDirection.left ? 0 : limit;
      return updateLastSelectionToHeaderSelection(focus: focus, axis: selection.axis);
    }
  ...
  /// * [TableShortcuts] and [TableActions] where the shortcuts are mapped
  /// into actions that allow the user to expand the last selection
  UserSelectionState moveActiveCell(IntVector2 Function(IntVector2 coordinate) getNextCoordinate) {
    final newAnchor = getNextCoordinate(primarySelection.anchorCoordinate);

    return resetSelectionsToACellSelection(anchor: newAnchor, focus: newAnchor);
  }
  ...
  /// * [TableShortcuts] and [TableActions] where the shortcuts are mapped
  /// into actions that allow the user to expand the last selection
  UserSelectionState moveActiveCellByBlock(IntVector2 Function(IntVector2 coordinate) getNextCoordinateInCellsBlock) {
    final newAnchor = getNextCoordinateInCellsBlock(primarySelection.anchorCoordinate);
  ...
  bool operator ==(Object other) =>
      ...
      other is UserSelectionState &&
          ...
          hasVisibleSelection == other.hasVisibleSelection &&
          _primaryIndex == other._primaryIndex;

  @override
  int get hashCode => selections.hashCode ^ hasVisibleSelection.hashCode ^ _primaryIndex.hashCode;
}
