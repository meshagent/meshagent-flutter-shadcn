...
class SelectionBorderSide {
  ...
  const SelectionBorderSide.solid({this.color, required this.width}) : dashed = false, dashIntervals = const <double>[];

  const SelectionBorderSide.dashed({this.color, required this.width, this.dashIntervals = kDefaultDashIntervals}) : dashed = true;

  const SelectionBorderSide.none() : dashed = false, dashIntervals = const <double>[], width = 0, color = null;
  ...
  BorderSide toFlutterBorderSide() {
    return BorderSide(color: color ?? const Color(0x00000000), width: width);
  }
  ...
  @override
  int get hashCode => color.hashCode ^ width.hashCode ^ dashed.hashCode ^ dashIntervals.hashCode;
}
...
class SelectionStyle {
  ...
  const SelectionStyle({required this.backgroundColor, required this.borderSide});
  ...
  SelectionStyle.semiTransparent({required Color color, double borderWidth = 2.0})
    : this(
        ...
        borderSide: SelectionBorderSide.solid(color: color, width: borderWidth),
      );
  ...
  SelectionStyle.dashedBorderOnly({required Color color, double borderWidth = 1.0})
    : this(
        ...
        borderSide: SelectionBorderSide.dashed(color: color, width: borderWidth),
      );

  /// Creates a copy of this object but with the given fields replaced with the
  /// new values.
  SelectionStyle copyWith({Color? backgroundColor, SelectionBorderSide? borderSide, bool? dashed}) {
    return SelectionStyle(backgroundColor: backgroundColor ?? this.backgroundColor, borderSide: borderSide ?? this.borderSide);
  }
