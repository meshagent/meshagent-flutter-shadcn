...
class SwayzeTableDataController<ParentType extends SwayzeController> extends DataController
    implements SubController<ParentType>, Listenable {
  @override
  ...
  /// Whether selections and gestures may extend the table past its real row
  /// and column counts.
  final bool allowElasticExpansion;

  /// Whether users may resize columns and rows by dragging header separators.
  final bool allowHeaderResize;
  ...
  Range2D get tableRange => Range2D.fromLTWH(const IntVector2.symmetric(0), IntVector2(columns.value.totalCount, rows.value.totalCount));

  /// The last valid index in the actual table on a given [axis].
  int getLastTableHeaderIndex({required Axis axis}) {
    final count = getHeaderControllerFor(axis: axis).value.count;
    return max(0, count - 1);
  }

  /// Clamp a header index to the actual table bounds.
  int clampHeaderIndexToTable({required Axis axis, required int index}) {
    return min(max(index, 0), getLastTableHeaderIndex(axis: axis));
  }

  /// Clamp a cell coordinate to the actual table bounds.
  IntVector2 clampCellCoordinateToTable(IntVector2 coordinate) {
    return IntVector2(
      clampHeaderIndexToTable(axis: Axis.horizontal, index: coordinate.dx),
      clampHeaderIndexToTable(axis: Axis.vertical, index: coordinate.dy),
    );
  }
  ...
  void handleSelectionChange() {
    if (!allowElasticExpansion) {
      final hasElasticColumns = columns.value.elasticCount > columns.value.count;
      final hasElasticRows = rows.value.elasticCount > rows.value.count;

      if (!hasElasticColumns && !hasElasticRows) {
        return;
      }

      scheduleMicrotask(() {
        columns.updateElasticCount(columns.value.count);
        rows.updateElasticCount(rows.value.count);
      });
      return;
    }

    final selections = [...parent.selection.userSelectionState.selections, ...parent.selection.dataSelections];

    final currentElasticEdge = IntVector2(columns.value.elasticCount, rows.value.elasticCount);

    final elasticEdge = selections.fold<IntVector2>(const IntVector2.symmetric(0), (acc, selection) {
      final rowEdge = selection.bottom;
      final columnEdge = selection.right;

      return acc.copyWith(x: columnEdge == null ? acc.dx : max(acc.dx, columnEdge), y: rowEdge == null ? acc.dy : max(acc.dy, rowEdge));
    });
...
class SwayzeHeaderController extends ValueNotifier<SwayzeHeaderState> implements ControllerBase {
  SwayzeHeaderController._({required SwayzeHeaderState initialState}) : super(initialState);

  void updateState(SwayzeHeaderState Function(SwayzeHeaderState previousState) stateUpdate) {
