...
class SwayzeHeaderState {
  ...
  late final customSizedHeaders = Map<int, SwayzeHeaderData>.unmodifiable(_customSizedHeaders);
  ...
  late final double extent = (() {
    ...
    final allCustomSizes = _customSizedHeaders.entries.fold<double>(0.0, (value, entry) {
    ...
    final allRegularSizes = (totalCount - customSizesCount) * defaultHeaderExtent;
  ...
  SwayzeHeaderState({
    ...
    int? elasticCount,
       ...
       _customSizedHeaders = headerData.fold(SplayTreeMap<int, SwayzeHeaderData>(), (previousValue, element) {
  ...
  SwayzeHeaderState copyWith({int? count, int? elasticCount, Iterable<SwayzeHeaderData>? headerData, int? frozenCount}) {
    if (headerData != null) {
  ...
  SwayzeHeaderState setHeaderExtent(int index, double extent) {
    ...
    final _newCustomSizedHeaders = SplayTreeMap<int, SwayzeHeaderData>.from(_customSizedHeaders);

    if (extent == defaultHeaderExtent) {
      _newCustomSizedHeaders.remove(index);
    } else if (currentHeaderData == null) {
      _newCustomSizedHeaders[index] = SwayzeHeaderData(index: index, extent: extent, hidden: false);
    } else {
      _newCustomSizedHeaders[index] = currentHeaderData.copyWith(extent: extent);
    }
  ...
  @override
  int get hashCode =>
      _frozenCount.hashCode ^ defaultHeaderExtent.hashCode ^ count.hashCode ^ elasticCount.hashCode ^ customSizedHeaders.hashCode;
...
class SwayzeHeaderData {
  ...
  const SwayzeHeaderData({required this.index, required this.extent, required this.hidden})
    : assert((hidden == true) || (extent != null), 'A SwayzeHeaderData either has to be hidden or have a custom extent');

  double get effectiveExtent => hidden ? 0.0 : extent!;

  SwayzeHeaderData copyWith({double? extent, bool? hidden}) {
    return SwayzeHeaderData(index: index, extent: extent ?? this.extent, hidden: hidden ?? this.hidden);
  }
