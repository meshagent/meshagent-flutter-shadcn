...
VirtualizationCalcResult pixelRangeToHeaderRange(double leadingPixel, double trailingPixel, HeaderComputationAuxiliaryData auxiliaryData) {
  ...
  final leadingIndex = leadingResult.headerIndexInPoint.clamp(0, auxiliaryData.lastHeaderIndex);

  // Clamp the trailing index into acceptable boundaries.
  final trailingIndex = trailingResult.headerIndexInPoint.clamp(0, auxiliaryData.lastHeaderIndex);
...
class VirtualizationCalcResult {
  ...
  /// A neutral result.
  static const VirtualizationCalcResult zero = VirtualizationCalcResult(0.0, Range.zero);
  ...
  bool operator ==(Object other) =>
      ...
      other is VirtualizationCalcResult && runtimeType == other.runtimeType && displacement == other.displacement && range == other.range;
...
_IndexFindersAccumulators _pixelToHeaderIndex({
  ...
  while (customSizesPointer < orderedKeys.length) {
    ...
    final regularHeadersIntervalExtent = regularHeadersIntervalCount * defaultSize;
    ...
    final trailingEdgeExtent = leadingEdgeExtent + (customSizesMap[customSizeKey]?.effectiveExtent ?? 0.0);
