...
class AutoScrollDragScrollData implements DragScrollData {
  ...
  const AutoScrollDragScrollData({required this.direction, required this.pointerDistance, this.maxToScroll});
}
...
DragScrollData getHorizontalDragScrollData({
  ...
  if (pointerDistanceToLeftThreshold > 0) {
    return AutoScrollDragScrollData(direction: AxisDirection.left, pointerDistance: pointerDistanceToLeftThreshold);
  }

  final rightThreshold = screenWidth - config.kRowHeaderWidth - kAutoScrollTriggerThreshold;
  final pointerDistanceToRightThreshold = globalOffset - rightThreshold;
  if (pointerDistanceToRightThreshold > 0) {
    return AutoScrollDragScrollData(direction: AxisDirection.right, pointerDistance: pointerDistanceToRightThreshold);
  }
