...
class _InlineEditorOverlayContent extends StatefulWidget {
  ...
  @override
  State<_InlineEditorOverlayContent> createState() => _InlineEditorOverlayContentState();
}

class _InlineEditorOverlayContentState extends State<_InlineEditorOverlayContent> {
  ...
  late final cell = internalScope.controller.cellsController.cellMatrixReadOnly[widget.cellCoordinate];
  ...
  Widget build(BuildContext context) {
    final borderSide = style.userSelectionStyle.borderSide.toFlutterBorderSide();
    ...
    return CustomSingleChildLayout(
      delegate: _PositionOverlayDelegate(editorRect: editorRect, contentAlignment: contentAlignment),
      child: IntrinsicWidth(
        child: IntrinsicHeight(
          child: DecoratedBox(
            decoration: BoxDecoration(border: border, boxShadow: style.inlineEditorShadow),
            child: Padding(
              ...
              child: Align(
                ...
                child: ConstrainedBox(
                  ...
                  child: _TableOverlapCalculator(
                    ...
                    builder: (context, overlapsTableRect, overlapsCellRect) {
...
class _PositionOverlayDelegate extends SingleChildLayoutDelegate {
  ...
  _PositionOverlayDelegate({required this.editorRect, required this.contentAlignment});
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    final aligmentLeft = alignRight ? editorRect.right - childSize.width : editorRect.left;

    final effectiveLeft = aligmentLeft.clamp(0.0, size.width - childSize.width);
    final effectiveTop = editorRect.top.clamp(0.0, size.height - childSize.height);
    return Offset(effectiveLeft, effectiveTop);
  }

  @override
  bool shouldRelayout(_PositionOverlayDelegate oldDelegate) {
    return oldDelegate.editorRect != editorRect || oldDelegate.contentAlignment != contentAlignment;
  }
}
...
class _TableOverlapCalculator extends StatefulWidget {
  final Widget Function(BuildContext context, bool overlapTable, bool overlapCell) builder;

  final Rect tableRect;
  final Rect cellRect;

  const _TableOverlapCalculator({Key? key, required this.builder, required this.tableRect, required this.cellRect}) : super(key: key);

  @override
  State<_TableOverlapCalculator> createState() => _TableOverlapCalculatorState();
}
...
class _TableOverlapCalculatorState extends State<_TableOverlapCalculator> {
  ...
  void updateRectPositioning() {
    ...
    final translation = contextRenderObjectRect.getTransformTo(overlay).getTranslation();

    final contextRect = contextRenderObjectRect.paintBounds.shift(Offset(translation.x, translation.y));

    setState(() {
      overlapsTableRect = widget.tableRect != Rect.zero ? widget.tableRect.overlaps(contextRect) : false;
      overlapsCellRect = widget.cellRect != Rect.zero ? widget.cellRect.overlaps(contextRect) : false;
    });
  }

  @override
  Widget build(BuildContext context) {
    return widget.builder(context, overlapsTableRect, overlapsCellRect);
  }
}
