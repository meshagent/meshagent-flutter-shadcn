...
class InlineEditorPlacer extends StatefulWidget {
  ...
  const InlineEditorPlacer({Key? key, required this.child, required this.inlineEditorBuilder}) : super(key: key);
...
class _InlineEditorPlacerState extends State<InlineEditorPlacer> {
  ...
  final rectNotifier = RectPositionsNotifier(cellRect: Rect.zero, tableRect: Rect.zero);
  ...
  void updateRectPositions() {
    ...
    final cellRect = _getCellRect(context: context, tableRect: tableRect, cellCoordinate: cellCoordinate) ?? Rect.zero;
...
Rect? _getTableRect(BuildContext context) {
  ...
  final topLeft = MatrixUtils.transformPoint(table.getTransformTo(overlay), Offset.zero);
...
Rect? _getCellRect({required BuildContext context, required Rect tableRect, required IntVector2 cellCoordinate}) {
  ...
  final overlay = (Overlay.of(context).context.findRenderObject()! as RenderBox).paintBounds;

  final displacedRect = cellPositionResult.leftTop & cellPositionResult.cellSize;

  final displacementLeft = viewportContext.columns.virtualizationState.displacement;
  ...
  final finalLeft = (headerWidth + displacedRect.left + displacementLeft + tableLeft).clamp(headerWidth, limitLeft);

  final finalTop = (headerHeight + displacedRect.top + displacementTop + tableTop).clamp(
