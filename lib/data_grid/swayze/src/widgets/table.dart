...
class SliverSwayzeTable<CellDataType extends SwayzeCellData> extends StatefulWidget {
  ...
  /// Physics used by the internal horizontal scroll view.
...
class SliverSwayzeTableState extends State<SliverSwayzeTable> {
  ...
  Widget build(BuildContext context) {
    final effectiveHeaderHeight = widget.stickyHeader == null ? 0.0 : widget.stickyHeaderSize!;

    final focusScopeNode = FocusScope.of(context);

    Widget child = SliverTwoAxisScroll(
      paddingTop: effectiveHeaderHeight, // the size of the header is compensated
      // in the sliver
          ...
          child: TableFocusProvider(
            ...
            child: DefaultActions(
              child: Focus(
                ...
                focusNode: widget.focusNode,
                child: InlineEditorPlacer(inlineEditorBuilder: widget.inlineEditorBuilder, child: child),
              ),
            ),
          ),
        );
    ...
    return InternalScopeProvider(cellDelegate: widget.cellDelegate, controller: widget.controller, style: widget.style, child: child);
  }
}
...
class _StickHeaderWrapper extends StatelessWidget {
  ...
  const _StickHeaderWrapper({Key? key, required this.headerHeight, required this.child, required this.headerState}) : super(key: key);
        ...
        return ValueListenableBuilder<SwayzeHeaderState>(
          ...
          builder: (context, value, _) {
            ...
            return UnconstrainedBox(
              ...
              child: SizedBox(
                width: min(constraints.maxWidth - _kStickyHeaderRightPadding, headerWidth),
                height: headerHeight,
                child: child,
              ),
            );
