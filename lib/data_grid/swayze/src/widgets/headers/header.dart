...
class Header extends StatelessWidget {
  ...
  const Header({Key? key, required this.axis, required this.displacement, required this.wrapHeader}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final pixelExtent = viewportContext.getAxisContextFor(axis: axis).value.extent;

    final style = InternalScope.of(context).style;
    final showLeadingOuterBorders = style.showLeadingOuterBorders;
    final crossAxisBorder = switch (axis) {
      Axis.horizontal => Border(
        top: showLeadingOuterBorders ? BorderSide(color: style.cellSeparatorColor, width: style.cellSeparatorStrokeWidth) : BorderSide.none,
        bottom: BorderSide(color: style.cellSeparatorColor, width: style.cellSeparatorStrokeWidth),
      ),
      Axis.vertical => Border(
        left: showLeadingOuterBorders
    ...
    return ClipRect(
      child: DecoratedBox(
        decoration: BoxDecoration(color: style.defaultHeaderPalette.background, border: crossAxisBorder),
        child: CustomSingleChildLayout(delegate: _HeaderLayoutDelegate(axis, displacement, pixelExtent), child: headerContent),
      ),
    );
  }
}
...
class _HeaderLayoutDelegate extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final maxHeight = axis == Axis.vertical ? pixelExtent : constraints.maxHeight;

    return BoxConstraints.tight(constraints.constrain(Size(maxWidth, maxHeight)));
  }
  ...
  @override
  int get hashCode => axis.hashCode ^ displacement.hashCode ^ pixelExtent.hashCode;
}
...
class _HeaderModeRangeLists {
  ...
  _HeaderModeRangeLists({required this.selectedRangeList, required this.highlightedRangeList});
}
...
/// by [HeaderStyleState]
_HeaderModeRangeLists _getHeaderModeRangeLists(Iterable<UserSelectionModel> selections, Axis axis, Range target) {
  ...
  for (final selection in selections) {
    if (selection is CellUserSelectionModel) {
      highlightedRangeList.add(axis == Axis.horizontal ? selection.xRange : selection.yRange);
    }

    if (selection is TableUserSelectionModel || (selection is HeaderUserSelectionModel && selection.axis != axis)) {
  ...
  return _HeaderModeRangeLists(selectedRangeList: selectedRangeList & target, highlightedRangeList: highlightedRangeList & target);
}
...
class _HeaderRangeSubscriberState extends State<_HeaderRangeSubscriber> {
  ...
  late final SwayzeSelectionController selectionController = internalScope.controller.selection;
  ...
  void initState() {
    ...
    selectionController.userSelectionsListenable.addListener(onSelectionsChange);
  }

  @override
  void dispose() {
    widget.rangeNotifier.removeListener(onRangeChange);
    selectionController.userSelectionsListenable.removeListener(onSelectionsChange);
    super.dispose();
  }
  ...
  ///
  /// Header selection cues remain visible even when the table temporarily
  /// loses focus, matching spreadsheet behavior during context menus and
  ...
  List<HeaderStyleState> _getHeadersHeaderStyleState() {
    ...
    final selections = selectionController.userSelectionState.visibleSelections;
    final result = <HeaderStyleState>[];

    // Add styles for frozen headers
    if (frozenCount > 0) {
      final frozenRangeLists = _getHeaderModeRangeLists(selections, widget.axis, Range(0, frozenCount));

      // Iterate on the frozen range and check the expected background mode
      for (var index = 0; index < frozenCount; index++) {
        if (frozenRangeLists.selectedRangeList.where((value) => value.contains(index)).isNotEmpty) {
          result.add(HeaderStyleState.selected);
        } else if (frozenRangeLists.highlightedRangeList.where((value) => value.contains(index)).isNotEmpty) {
    ...
    for (final index in range.iterable) {
      if (rangeLists.selectedRangeList.where((value) => value.contains(index)).isNotEmpty) {
        result.add(HeaderStyleState.selected);
      } else if (rangeLists.highlightedRangeList.where((value) => value.contains(index)).isNotEmpty) {
        result.add(HeaderStyleState.highlighted);
      } else {
        result.add(HeaderStyleState.normal);
      }
  ...
  Widget build(BuildContext context) {
    ...
    return ExpandAll(
      children: [
        ...
        HeaderGestureDetector(axis: widget.axis, displacement: widget.displacement),
      ],
    );
  }
}
