...
class HeaderDisplacer extends MultiChildRenderObjectWidget {
  ...
  RenderObject createRenderObject(BuildContext context) {
    return _RenderHeaderDisplacer(frozenExtent: frozenExtent, frozenCount: frozenCount, displacement: displacement, background: background)
      ..axis = axis;
  }

  @override
  void updateRenderObject(BuildContext context, _RenderHeaderDisplacer renderObject) {
    renderObject
      ..axis = axis
...
class _RenderHeaderDisplacer extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, _HeaderDisplacerData>, RenderBoxContainerDefaultsMixin<RenderBox, _HeaderDisplacerData> {
    ...
    switch (_axis) {
      case Axis.horizontal:
        ...
        while (child != null) {
          ...
          if (childIndex < frozenCount) {
            childParentData.offset = Offset(mainAxisExtent + displacement.abs(), 0.0);
          } else {
      ...
      case Axis.vertical:
        ...
        while (child != null) {
          ...
          // compensating the displacement.
          if (childIndex < frozenCount) {
            childParentData.offset = Offset(0.0, mainAxisExtent + displacement.abs());
          } else {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    while (child != null) {
      ...
      if (childIndex == frozenCount && frozenCount > 0) {
        ...
        final width = axis == Axis.horizontal ? max(backgroundExtent, 0.0) : size.width;
        final height = axis == Axis.vertical ? max(backgroundExtent, 0.0) : size.height;

        context.canvas.drawRect(offset & Size(width, height), Paint()..color = background);
      }
      childIndex--;
    }
  }
