...
class MouseHoverTableBody extends StatefulWidget {
  ...
  const MouseHoverTableBody({Key? key, required this.child}) : super(key: key);

  static KeyedNotifier<IntVector2> of(BuildContext context) {
    return context.findAncestorWidgetOfExactType<_TableBodyMouseHoverProvider>()!.hoverNotifier;
  }
...
class _MouseHoverTableBodyState extends State<MouseHoverTableBody> {
  ...
  void onPointerHover(PointerHoverEvent event) {
    ...
    _debounce = Timer(const Duration(milliseconds: _kMouseHoverDebounceMilliseconds), () => updateMouseHoverNotifier(localPosition));
  }

  void updateMouseHoverNotifier(Offset localPosition) {
    final column = viewportContext.pixelToPosition(localPosition.dx, Axis.horizontal);
    final row = viewportContext.pixelToPosition(localPosition.dy, Axis.vertical);
  ...
  Widget build(BuildContext context) => MouseRegion(
    ...
    opaque: false,
    child: _TableBodyMouseHoverProvider(hoverNotifier: mouseHoverNotifier, child: widget.child),
  );
}
...
class _TableBodyMouseHoverProvider extends InheritedWidget {
  ...
  const _TableBodyMouseHoverProvider({Key? key, required this.hoverNotifier, required Widget child}) : super(key: key, child: child);
