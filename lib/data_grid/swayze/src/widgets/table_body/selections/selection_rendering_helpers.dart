...
class SelectionBorder {
  ...
  @override
  int get hashCode => borderSide.hashCode ^ hasLeft.hashCode ^ hasTop.hashCode ^ hasRight.hashCode ^ hasBottom.hashCode;
}
...
class SelectionRenderData {
  ...
  const SelectionRenderData({required this.rect, required this.border, this.backgroundColor});

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is SelectionRenderData && runtimeType == other.runtimeType && rect == other.rect && border == other.border;
...
mixin SelectionRenderingHelpers {
  ...
  SelectionBorder getVisibleBorder(Range2D range, SelectionBorderSide borderSide) {
    final hasLeft = xRange.contains(range.leftTop.dx);
    final hasTop = yRange.contains(range.leftTop.dy);
    final hasRight = xRange.contains(range.rightBottom.dx) || xRange.end == range.rightBottom.dx;
    final hasBottom = yRange.contains(range.rightBottom.dy) || yRange.end == range.rightBottom.dy;

    return SelectionBorder(borderSide: borderSide, hasLeft: hasLeft, hasTop: hasTop, hasRight: hasRight, hasBottom: hasBottom);
  }

  Offset getLeftTopOffset(IntVector2 coordinate) {
    final x = viewportContext.positionToPixel(coordinate.dx, Axis.horizontal, isForFrozenPanes: isOnFrozenColumns).pixel;
    final y = viewportContext.positionToPixel(coordinate.dy, Axis.vertical, isForFrozenPanes: isOnFrozenRows).pixel;
...
void paintSelectionBorder(Canvas canvas, Rect selectionRect, SelectionBorder border) {
  ...
  if (border.borderSide.dashed) {
    path = dashPath(path, dashArray: CircularIntervalList(border.borderSide.dashIntervals));
  }
