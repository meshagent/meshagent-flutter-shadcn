...
class SecondarySelectionGroup extends LeafRenderObjectWidget {
  ...
  const SecondarySelectionGroup({Key? key, required this.selectionGroup, required this.selectionStyle, required this.activeCellRect})
    : super(key: key);

  @override
  RenderObject createRenderObject(BuildContext context) => RenderSelectionOverlapCombiner(selectionGroup, selectionStyle, activeCellRect);

  @override
  void updateRenderObject(BuildContext context, RenderSelectionOverlapCombiner renderObject) {
    renderObject
...
class RenderSelectionOverlapCombiner extends RenderBox {
  RenderSelectionOverlapCombiner(this._selectionGroup, this._selectionStyle, this._activeCellRect);
  ...
  late final backgroundPaint = CachedValue(
    () => Paint()..color = _selectionStyle.backgroundColor ?? const Color(0x00000000),
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_selectionGroup.length <= kOverlapThreshold) {
      ...
      for (final selectionRenderData in _selectionGroup) {
        final selectionRect = selectionRenderData.rect;
        final selectionPath = Path()..addRect(selectionRect);
        backgroundPath = Path.combine(PathOperation.union, backgroundPath, selectionPath);

        paintSelectionBorder(canvas, selectionRect, selectionRenderData.border);
      }

      final overallPath = Path.combine(PathOperation.difference, backgroundPath, activeCellPath);
