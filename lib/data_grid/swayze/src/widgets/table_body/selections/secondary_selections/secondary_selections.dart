...
class _SecondarySelectionsState extends State<SecondarySelections> with SelectionRenderingHelpers {
  @override
  ...
  late final tableController = InternalScope.of(context).controller.tableDataController;

  late Map<SelectionStyle, Iterable<SelectionRenderData>> selectionsGroupedByStyle;
  ...
  void groupSelectionModelsByStyle() {
    ...
    for (final userSelectionModel in secondarySelections) {
      ...
      if (userSelectionModel is CellUserSelectionModel && !viewportRange2D.overlaps(userSelectionModel)) {
        continue;
      }

      if (userSelectionModel is HeaderUserSelectionModel) {
        final rangeAxis = userSelectionModel.axis == Axis.horizontal ? xRange : yRange;
      ...
      selectionList.add(SelectionRenderData(rect: leftTopPixelOffset & size, border: getVisibleBorder(range, borderSide)));
  ...
  Widget build(BuildContext context) {
    ...
    selectionsGroupedByStyle.forEach((selectionStyle, selectionRenderData) {
      children.add(
        SecondarySelectionGroup(selectionStyle: selectionStyle, selectionGroup: selectionRenderData, activeCellRect: widget.activeCellRect),
      );
    });

    return ExpandAll(children: children);
  }
}
