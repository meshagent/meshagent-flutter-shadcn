...
class TableBodySelections extends StatelessWidget {
  ...
  const TableBodySelections({Key? key, this.isOnAFrozenColumnsArea = false, this.isOnAFrozenRowsArea = false}) : super(key: key);
...
class _TableBodySelectionsState extends State<_TableBodySelections> {
  ...
  ///
  /// When a user moves to another element, keep the current selection but
  /// clamp it back to the real table bounds so elastic expansion can retract
  /// without resetting the user back to the initial cell.
  void handleFocusChanged() {
    if (!tableFocus.value.isActive) {
      ...
      widget.selectionController.updateUserSelections((state) => _clampSelectionStateToTable(state, tableDataController));
    }
  }

  @override
  Widget build(BuildContext context) {
    final userSelectionState = widget.selectionController.userSelectionState;
    final dataSelections = widget.selectionController.dataSelections;
    final visibleSelections = userSelectionState.visibleSelections.toList(growable: false);
    ...
    if (visibleSelections.isNotEmpty) {
      ...
      final positionActiveCell = viewportContext.getCellPosition(userSelectionState.activeCellCoordinate);
      final activeCellRect = positionActiveCell.leftTop & positionActiveCell.cellSize;
...
UserSelectionState _clampSelectionStateToTable(UserSelectionState state, SwayzeTableDataController tableDataController) {
  var changed = false;
  final clampedSelections = [
    for (final selection in state.selections)
      switch (selection) {
        CellUserSelectionModel() => _clampCellSelection(selection, tableDataController, onChange: () => changed = true),
        HeaderUserSelectionModel() => _clampHeaderSelection(selection, tableDataController, onChange: () => changed = true),
        TableUserSelectionModel() => selection,
        _ => selection,
      },
  ];

  return changed ? state.replaceSelections(clampedSelections) : state;
}

CellUserSelectionModel _clampCellSelection(
  CellUserSelectionModel selection,
  SwayzeTableDataController tableDataController, {
  required VoidCallback onChange,
}) {
  final clampedAnchor = tableDataController.clampCellCoordinateToTable(selection.anchor);
  final clampedFocus = tableDataController.clampCellCoordinateToTable(selection.focus);

  if (clampedAnchor == selection.anchor && clampedFocus == selection.focus) {
    return selection;
  }

  onChange();
  return selection.copyWith(anchor: clampedAnchor, focus: clampedFocus);
}

HeaderUserSelectionModel _clampHeaderSelection(
  HeaderUserSelectionModel selection,
  SwayzeTableDataController tableDataController, {
  required VoidCallback onChange,
}) {
  final clampedAnchor = tableDataController.clampHeaderIndexToTable(axis: selection.axis, index: selection.anchor);
  final clampedFocus = tableDataController.clampHeaderIndexToTable(axis: selection.axis, index: selection.focus);
