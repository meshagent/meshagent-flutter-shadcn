...
class _CellsElement<CellDataType extends SwayzeCellData> extends RenderObjectElement {
  ...
  SwayzeCellsController<CellDataType> get cellsController => widget.swayzeController.cellsController as SwayzeCellsController<CellDataType>;

  @override
  Future<void> insertRenderObjectChild(RenderBox child, IntVector2 slot) async {
    renderObject.insert(child, slot);
    assert(child.parent == renderObject);
  }

  @override
  void moveRenderObjectChild(RenderBox child, IntVector2 oldSlot, IntVector2 newSlot) {
    assert(child.parent == renderObject);
  ...
  void mount(Element? parent, dynamic newSlot) {
    ...
    retrieveDataAndCreateChildren(colIndices: widget.visibleColumnsIndices, rowIndices: widget.visibleRowsIndices);
  }
  ...
  void markNeedsCellStoreSync(List<CellOperation> lastPerformedOperations) {
    scheduleMicrotask(() {
      ...
      final viewport = Range2D.fromSides(widget.horizontalRange, widget.verticalRange);
  ...
  void createChild(CellDataType cell) {
    ...
    final offset = Offset(widget.columnOffsets[localColIndex], widget.rowOffsets[localRowIndex]);
    final size = Size(widget.columnSizes[localColIndex], widget.rowSizes[localRowIndex]);
    ...
    final parentData = CellParentData(offset: offset, size: size, position: localPosition, updateStamp: lastControllerSync);
  ...
  void retrieveDataAndCreateChildren({required Iterable<int> colIndices, required Iterable<int> rowIndices}) =>
  ...
  bool checkAndHandleScrollUpdates({required Cells oldWidget}) {
    ...
    if (beforeHorizontal == afterHorizontal && beforeVertical == afterVertical) {
      return false;
    }
    ...
    if (rowsIndicesToRemove.isNotEmpty || columnsIndicesToRemove.isNotEmpty) {
      for (final rowIndex in beforeVertical) {
        ...
        children.removeWhereInRow(rowIndex, (colIndex, child) => columnsIndicesToRemove.contains(colIndex));
      }
    }
    ...
    if (rowsIndicesToAdd.isNotEmpty || columnsIndicesToAdd.isNotEmpty) {
      for (final rowIndex in afterVertical) {
        ...
        if (rowsIndicesToAdd.contains(rowIndex)) {
          retrieveDataAndCreateChildren(rowIndices: [rowIndex], colIndices: widget.visibleColumnsIndices);
        } else {
          retrieveDataAndCreateChildren(rowIndices: [rowIndex], colIndices: columnsIndicesToAdd);
        }
      }
    }
  ...
  bool checkAndHandleHeaderUpdates({required Cells oldWidget}) {
    ...
    if (_kRangeListEquality.equals(beforeSizesHorizontal, afterSizesHorizontal) &&
        _kRangeListEquality.equals(beforeSizesVertical, afterSizesVertical)) {
      return false;
    }
      ...
      iterate: (cell, colIndex, rowIndex) {
        ...
        if (existingChild != null) {
          ...
          if (widget.data == cell) {
            ...
            (existingChild.renderObject!.parentData! as CellParentData).updateStamp = lastControllerSync;
            return;
          }
          ...
          final newChild = updateChild(existingChild, childWidget, cell.position)!;

          newChild.renderObject!.parentData = existingParentData;

          // mark new child as up to date
          (newChild.renderObject!.parentData! as CellParentData).updateStamp = lastControllerSync;
...
class _RenderCells extends RenderBox {
  ...
  void remove(RenderBox child) {
    ...
    children.remove(colIndex: data.dx, rowIndex: data.dy);
    dropChild(child);
  }
  ...
  void move(RenderBox child, IntVector2 from, IntVector2 to) {
    ...
    assert(children[from] != child, 'Tried to move a child from a different place.');
    children.remove(colIndex: from.dx, rowIndex: from.dy);
  ...
  void performLayout() {
    children.forEach((child, colIndex, rowIndex) {
      child.layout(BoxConstraints.loose((child.parentData as CellParentData?)!.size));
    });
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    children.forEach((child, colIndex, rowIndex) {
      final childOffset = (child.parentData as CellParentData?)!.offset + offset;
      context.pushLayer(OffsetLayer(offset: childOffset), (PaintingContext context, Offset offset) {
...
class CellParentData extends ContainerBoxParentData<_RenderCells> {
  ...
  CellParentData({required Offset offset, required this.size, required this.position, required this.updateStamp}) {
    this.offset = offset;
  }
