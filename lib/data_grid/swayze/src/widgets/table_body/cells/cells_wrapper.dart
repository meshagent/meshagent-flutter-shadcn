...
class CellsWrapper extends StatefulWidget {
  ...
  const CellsWrapper({Key? key, this.isOnAFrozenColumnsArea = false, this.isOnAFrozenRowsArea = false}) : super(key: key);
...
class _CellsWrapperState extends State<CellsWrapper> {
  ...
  Widget build(BuildContext context) {
    ...
    // Ranges
    final horizontalRange = isOnFrozenColumns ? columnsState.frozenRange : columnsState.scrollableRange;
    final verticalRange = isOnFrozenRows ? rowsState.frozenRange : rowsState.scrollableRange;

    // Offsets
    final columnOffsets = isOnFrozenColumns ? columnsState.frozenOffsets : columnsState.offsets;
    final rowOffsets = isOnFrozenRows ? rowsState.frozenOffsets : rowsState.offsets;

    final columnSizes = isOnFrozenColumns ? columnsState.frozenSizes : columnsState.sizes;
    final rowSizes = isOnFrozenRows ? rowsState.frozenSizes : rowsState.sizes;

    // Visible indices
    final visibleColumnsIndices = isOnFrozenColumns ? columnsState.visibleFrozenIndices : columnsState.visibleIndices;
    final visibleRowsIndices = isOnFrozenRows ? rowsState.visibleFrozenIndices : rowsState.visibleIndices;
