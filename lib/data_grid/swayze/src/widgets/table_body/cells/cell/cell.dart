...
class _CellState<CellDataType extends SwayzeCellData> extends State<Cell<CellDataType>> {
  late final mouseHover = MouseHoverTableBody.of(context);

  late bool isActive = widget.selectionsController.userSelectionState.activeCellCoordinate == widget.data.position;
  ...
  void initState() {
    ...
    if (isActiveCellAware) {
      widget.selectionsController.userSelectionsListenable.addListener(handleSelectionChanged);
    }
  }
  ...
  void didUpdateWidget(Cell<CellDataType> oldWidget) {
    ...
    if (wasActiveCellAware) {
      oldWidget.selectionsController.userSelectionsListenable.removeListener(handleSelectionChanged);
    }
  ...
  void dispose() {
    final isActiveCellAware = cellLayout.isActiveCellAware;
    if (isActiveCellAware) {
      widget.selectionsController.userSelectionsListenable.removeListener(handleSelectionChanged);
    }
  ...
  void handleSelectionChanged() {
    final activeCellCoordinate = widget.selectionsController.userSelectionState.activeCellCoordinate;
      ...
      return KeyedNotifierBuilder<IntVector2>(
        ...
        builder: (context, isHover) {
          return CellRootPainter(
            ...
            cellContent: cellLayout.buildCell(context, isHover: isHover, isActive: isActive),
            cellHoverWidgets: cellLayout.buildOverlayWidgets(context, isHover: isHover, isActive: isActive),
          );
        },
      );
    }
