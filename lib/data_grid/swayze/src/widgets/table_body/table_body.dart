...
class TableBody extends StatelessWidget {
  ...
  const TableBody({Key? key, required this.horizontalDisplacement, required this.verticalDisplacement, required this.wrapTableBody})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    Widget tableBody = MouseHoverTableBody(
      child: ExpandAll(
        children: [
          ...
          _TableBodyScrollableArea(style: style, viewportContext: viewportContext),
          ...
          TableBodyGestureDetector(horizontalDisplacement: horizontalDisplacement, verticalDisplacement: verticalDisplacement),
        ],
      ),
    );
    ...
    return ClipRect(
      child: CustomSingleChildLayout(
        delegate: _TableBodyLayoutDelegate(horizontalDisplacement, verticalDisplacement, areaSize),
        child: tableBody,
      ),
    );
  }
...
class _TableBodyScrollableArea extends StatelessWidget {
  ...
  const _TableBodyScrollableArea({Key? key, required this.style, required this.viewportContext}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final offset = Offset(viewportContext.columns.value.frozenExtent, viewportContext.rows.value.frozenExtent);

    return ExpandAll(
      children: [
        ColoredBox(color: style.defaultCellBackground),
        const RepaintBoundary(key: ValueKey('RepaintBoundaryCells'), child: CellsWrapper()),
        RepaintBoundary(
...
class _TableBodyFrozenArea extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final horizontalDisplacement = isOnAFrozenColumnsArea ? this.horizontalDisplacement.abs() : 0.0;

    final verticalDisplacement = isOnAFrozenRowsArea ? this.verticalDisplacement.abs() : 0.0;

    // Extent
    final horizontalExtent = isOnAFrozenColumnsArea ? viewportColumnsState.frozenExtent : viewportColumnsState.extent;

    final verticalExtent = isOnAFrozenRowsArea ? viewportRowsState.frozenExtent : viewportRowsState.extent;

    // Sizes
    final horizontalSizes = isOnAFrozenColumnsArea ? viewportColumnsState.frozenSizes : viewportColumnsState.sizes;

    final verticalSizes = isOnAFrozenRowsArea ? viewportRowsState.frozenSizes : viewportRowsState.sizes;
    ...
    return CustomSingleChildLayout(
      delegate: _TableBodyLayoutDelegate(horizontalDisplacement, verticalDisplacement, Size(horizontalExtent, verticalExtent)),
      child: ExpandAll(
        children: [
          ColoredBox(color: style.defaultCellBackground),
          RepaintBoundary(
            key: const ValueKey('RepaintBoundaryCells'),
            child: CellsWrapper(isOnAFrozenRowsArea: isOnAFrozenRowsArea, isOnAFrozenColumnsArea: isOnAFrozenColumnsArea),
          ),
          RepaintBoundary(
            key: const ValueKey('RepaintBoundaryTableLines'),
            child: TableLines(columnSizes: horizontalSizes, rowSizes: verticalSizes, swayzeStyle: style, translateOffset: offset),
          ),
          ClipRect(
            child: TableBodySelections(isOnAFrozenRowsArea: isOnAFrozenRowsArea, isOnAFrozenColumnsArea: isOnAFrozenColumnsArea),
          ),
        ],
      ),
    );
...
class _TableBodyLayoutDelegate extends SingleChildLayoutDelegate {
  ...
  const _TableBodyLayoutDelegate(this.horizontalDisplacement, this.verticalDisplacement, this.areaSize);
  ...
  @override
  int get hashCode => verticalDisplacement.hashCode ^ horizontalDisplacement.hashCode ^ areaSize.hashCode;
}
