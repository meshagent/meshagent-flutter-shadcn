...
class TableLines extends StatelessWidget {
  ...
  const TableLines({Key? key, required this.columnSizes, required this.rowSizes, required this.swayzeStyle, required this.translateOffset})
    : super(key: key);
...
class _TableLinesPainter extends LeafRenderObjectWidget {
  ...
  RenderObject createRenderObject(BuildContext context) {
    return _RenderTableAreaLines(columnSizes, rowSizes, lineColor, lineWidth, translateOffset);
  }

  @override
  void updateRenderObject(BuildContext context, _RenderTableAreaLines renderObject) {
    renderObject
      ..columnSizes = columnSizes
      ..rowSizes = rowSizes
      ..lineWidth = lineWidth
...
class _RenderTableAreaLines extends RenderBox {
  ...
  _RenderTableAreaLines(this._columnSizes, this._rowSizes, this._lineColor, this._lineWidth, this._translateOffset);
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (var i = 0; i < _columnSizes.length; i++) {
      ...
      canvas.drawLine(Offset.zero, Offset(0, size.height), linePaintCache.value);
    }
    canvas.restore();
    ...
    for (var index = 0; index < _rowSizes.length; index++) {
      ...
      canvas.drawLine(Offset.zero, Offset(size.width, 0), linePaintCache.value);
    }
    canvas.restore();
  }
}
