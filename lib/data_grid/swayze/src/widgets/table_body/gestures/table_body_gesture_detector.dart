...
class _TableGestureDetails {
  ...
  const _TableGestureDetails({required this.localPosition, required this.cellCoordinate});
...
_TableGestureDetails _getTableGestureDetails(BuildContext context, Offset globalPosition) {
  ...
  /// [axis].
  int _getCoordinateWithAditionalOffset({required Axis axis, required PositionResult positionResult, required double localPosition}) {
    var result = positionResult.position;
    if (positionResult.overflow == OffscreenDetails.trailing) {
      final diff = localPosition - viewportContext.getAxisContextFor(axis: axis).value.extent;
    ...
    if (!tableDataController.allowElasticExpansion) {
      return tableDataController.clampHeaderIndexToTable(axis: axis, index: result);
    }
  ...
  final positionResultX = viewportContext.pixelToPosition(localPosition.dx, Axis.horizontal);

  final positionResultY = viewportContext.pixelToPosition(localPosition.dy, Axis.vertical);

  return _TableGestureDetails(
    localPosition: localPosition,
    cellCoordinate: IntVector2(
      _getCoordinateWithAditionalOffset(axis: Axis.horizontal, positionResult: positionResultX, localPosition: localPosition.dx),
      _getCoordinateWithAditionalOffset(axis: Axis.vertical, positionResult: positionResultY, localPosition: localPosition.dy),
    ),
  );
}
...
class TableBodyGestureDetector extends StatefulWidget {
  ...
  const TableBodyGestureDetector({Key? key, required this.horizontalDisplacement, required this.verticalDisplacement}) : super(key: key);

  @override
  _TableBodyGestureDetectorState createState() => _TableBodyGestureDetectorState();
}
...
class _TableBodyGestureDetectorState extends State<TableBodyGestureDetector> {
  ...
  /// Tracks the body cell under the latest pointer down so taps can select a
  /// single cell while drags still select cell ranges.
  IntVector2? tapDownCoordinateCache;

  /// Tracks whether the current pointer sequence started from a primary-button
  /// interaction that this detector should complete on pointer up.
  bool isTrackingPrimaryPointerSequence = false;

  bool didStartDragSelection = false;
  ...
  void _resetGestureState() {
    tapDownCoordinateCache = null;
    isTrackingPrimaryPointerSequence = false;
    _stopDragSelection();
  }
  ...
  void onRangesChanged() {
    ...
    final horizontalScrollPosition = scrollController.horizontalScrollController!.position;
    final verticalScrollPosition = scrollController.verticalScrollController!.position;

    handleDragUpdate(
      IntVector2(
        horizontalScrollPosition.userScrollDirection != ScrollDirection.idle
            ? _getRangeEdgeOnAutoScroll(viewportContext.columns.value.scrollableRange, horizontalScrollPosition.userScrollDirection)
            : cachedDragCellCoordinate.dx,
        verticalScrollPosition.userScrollDirection != ScrollDirection.idle
            ? _getRangeEdgeOnAutoScroll(viewportContext.rows.value.scrollableRange, verticalScrollPosition.userScrollDirection)
  ...
  void updateDragScroll({
    ...
    final horizontalDragScrollData = getHorizontalDragScrollData(
      ...
      viewportExtent: viewportContext.columns.virtualizationState.scrollingData.viewportExtent,
    ...
    if (horizontalDragScrollData is AutoScrollDragScrollData) {
      ...
      scrollController.jumpToHeader(viewportContext.columns.value.frozenRange.end, Axis.horizontal);
    } else {
      // do not scroll
      scrollController.stopAutoScroll(Axis.horizontal);
    }
    ...
    if (verticalDragScrollData is AutoScrollDragScrollData) {
      ...
      scrollController.jumpToHeader(viewportContext.rows.value.frozenRange.end, Axis.vertical);
    } else {
      scrollController.stopAutoScroll(Axis.vertical);
    }
  }

  /// Handles drag starts that should start a cell-range selection.
  ...
  void handleStartSelection(_TableGestureDetails details) {
    Actions.invoke(context, TableBodySelectionStartIntent(details.cellCoordinate));
  }

  /// Handles taps that should select a single cell in the body.
  void handleTapSelection(IntVector2 cellCoordinate) {
    Actions.invoke(context, TableBodySelectionStartIntent(cellCoordinate));
  }
      ...
      builder: (context, constraints) {
        ...
        return Listener(
          ...
          onPointerDown: (details) {
            ...
            final tableGestureDetails = _getTableGestureDetails(context, details.position);

            tapDownCoordinateCache = tableGestureDetails.cellCoordinate;
            isTrackingPrimaryPointerSequence = true;
            didStartDragSelection = false;
          },
          onPointerUp: (_) {
            if (!isTrackingPrimaryPointerSequence) {
              _resetGestureState();
              return;
            }
            final tapDownCoordinateCache = this.tapDownCoordinateCache;
            if (!didStartDragSelection && tapDownCoordinateCache != null) {
              handleTapSelection(tapDownCoordinateCache);
            }
            _resetGestureState();
          },
          onPointerCancel: (_) {
                ...
                (PanGestureRecognizer instance) {
                  instance
                    ..onStart = (DragStartDetails details) {
                      final tableGestureDetails = _getTableGestureDetails(context, details.globalPosition);
                      final dragStartCoordinate = tapDownCoordinateCache ?? tableGestureDetails.cellCoordinate;
                      didStartDragSelection = true;
                      cachedDragCellCoordinate = dragStartCoordinate;
                      handleStartSelection(
                        _TableGestureDetails(localPosition: tableGestureDetails.localPosition, cellCoordinate: dragStartCoordinate),
                      );
                    ...
                    ..onUpdate = (DragUpdateDetails details) {
                      ...
                      cachedDragCellCoordinate = tableGestureDetails.cellCoordinate;
                      updateDragScroll(
                ...
                (DoubleTapGestureRecognizer instance) {
                  instance
                    ..onDoubleTapDown = (TapDownDetails details) {
                      ...
                      final gestureCoordinate = _getTableGestureDetails(context, details.globalPosition).cellCoordinate;
                      ...
                      Actions.invoke(context, OpenInlineEditorIntent(cellPosition: gestureCoordinate));
                    };
                },
              ),
            },
