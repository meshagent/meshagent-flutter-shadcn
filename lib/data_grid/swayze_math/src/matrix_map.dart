...
abstract class MatrixMapReadOnly<T> {
  ...
  void forEach(MatrixMapIterator<T> iterate);
  ...
  void forEachExistingItemOn({required Iterable<int> colIndices, required Iterable<int> rowIndices, required MatrixMapIterator<T> iterate});
  ...
  Iterable<MatrixMapIterableResult<T?>> forEachInRange({required Iterable<int> colIndices, required Iterable<int> rowIndices});

  /// Iterate in all items of this matrix map in a specific row.
  void forEachInRow(int rowIndex, MatrixMapIterator<T> iterate);
}
...
class MatrixMap<T> implements MatrixMapReadOnly<T> {
  ...
  @override
  T? operator [](IntVector2 coordinate) => elementAt(colIndex: coordinate.dx, rowIndex: coordinate.dy);
  ...
  @override
  Iterable<MatrixMapIterableResult<T?>> forEachInRange({required Iterable<int> colIndices, required Iterable<int> rowIndices}) sync* {
    for (final rowIndex in rowIndices) {
      final row = _rows[rowIndex];
      for (final colIndex in colIndices) {
        final cell = row?[colIndex];

        yield MatrixMapIterableResult(position: IntVector2(colIndex, rowIndex), value: cell);
      }
    }
  }
  ...
  void removeWhere(bool Function(int colIndex, int rowIndex, T value) predicate) {
    _rows.forEach((rowIndex, column) {
      column.removeWhere((colIndex, value) => predicate(colIndex, rowIndex, value));
    });
  }
